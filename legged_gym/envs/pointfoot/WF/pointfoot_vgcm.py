...
class PointFootVGCM:
    ...
    def _compute_torques(self, actions):
        ...
        if control_type == "P_AND_V":
            torques = self.p_gains * (
                    action_scale_pos * actions[:, :self.num_dof] + self.default_dof_pos - self.dof_pos) - self.d_gains * self.dof_vel
        ...
        self.vgcm_torques = self._update_vgcms(actions)
        torques -= self.vgcm_torques
        self.actuator_torques = torch.clip(torques, -self.torque_limits, self.torque_limits)
        return self.actuator_torques + self.vgcm_torques

    def _update_vgcms(self, actions):
        """ Update VGCMs based on commanded actions and delta T
        [NOTE]: resulting vector includes joints without VGCMs - i.e. wheels

        Returns
            [torch.Tensor]: Torques resulting from compensators
        """
        idxs = self.vgcm_idxs
        self.vgcm_tks[:, idxs] = torch.clip(
            self.default_vgcm_k[:, idxs] + actions[:, 8:14] * self.vgcm_k_range[idxs],
            self.vgcm_k_min[idxs],
            self.vgcm_k_max[idxs])
        self.vgcm_txs[:, idxs] = torch.clip(
            self.default_vgcm_x[:, idxs] + actions[:, 14:20] * self.vgcm_x_range[idxs],
            self.vgcm_x_min[idxs],
        ...
        delta_x = self.dof_pos - self.vgcm_xs
        return self.vgcm_ks * delta_x
    ...
    def _reset_dofs(self, env_ids):
        ...
        # Reset VGCM parameters
        alpha_min, alpha_max = self.cfg.init_state.alpha_range
        self.vgcm_alphas[env_ids] = torch_rand_float(
            alpha_min, alpha_max, (len(env_ids), 2), device=self.device)
        self.vgcm_dk[env_ids] = (self.vgcm_k_range / self.vgcm_alphas[env_ids, 0].unsqueeze(-1)).squeeze()
        self.vgcm_dx[env_ids] = (self.vgcm_x_range / self.vgcm_alphas[env_ids, 1].unsqueeze(-1)).squeeze()
        self.vgcm_ks[env_ids] = self.default_vgcm_k + torch_rand_float(-0.5, 0.5, (len(env_ids), self.num_dof),
                                                                       device=self.device) * self.vgcm_k_range
        self.vgcm_xs[env_ids] = torch.clip(
            self.dof_pos[env_ids] + torch_rand_float(-0.1, 0.1, (len(env_ids), self.num_dof),
                                                     device=self.device),
            self.vgcm_x_min,
            self.vgcm_x_max
        )
        self.vgcm_tks[env_ids] = self.vgcm_ks[env_ids]
        self.vgcm_txs[env_ids] = self.vgcm_xs[env_ids]
    ...
    def _get_noise_scale_vec(self):
        ...
        dof_pos_end_idx = command_end_idx + self.num_dof - 2 # ignore wheel pos
        ...
        # No noise on alpha
        vgcm_k_start_idx = last_action_end_idx + 2
        vgcm_k_end_idx = vgcm_k_start_idx + 6
        # Noise on k
        obs_noise_vec[vgcm_k_start_idx:vgcm_k_end_idx] = noise_scales.vgcm_k * noise_level * self.obs_scales.vgcm_k
        # Noise on x
        vgcm_x_end_idx = vgcm_k_end_idx + 6
        obs_noise_vec[vgcm_k_end_idx:vgcm_x_end_idx] = noise_scales.vgcm_x * noise_level * self.obs_scales.vgcm_x
    ...
    def _init_buffers(self):
        ...
        self.torques = torch.zeros(self.num_envs, self.num_dofs, dtype=torch.float, device=self.device,
                                   requires_grad=False)
        self.actuator_torques = torch.zeros(self.num_envs, self.num_dofs, dtype=torch.float, device=self.device,
                                   requires_grad=False)
        self.vgcm_torques = torch.zeros(self.num_envs, self.num_dofs, dtype=torch.float, device=self.device,
                                   requires_grad=False)
        ...
        # VGCM stuff
        self.vgcm_ks = torch.zeros((self.num_envs, self.num_dof), dtype=torch.float, device=self.device, requires_grad=False)
        self.vgcm_xs = torch.zeros_like(self.vgcm_ks)
        self.vgcm_tks = torch.zeros_like(self.vgcm_ks)
        self.vgcm_txs = torch.zeros_like(self.vgcm_xs)
        self.vgcm_dk = torch.zeros_like(self.vgcm_ks)
        self.vgcm_dx = torch.zeros_like(self.vgcm_xs)
        self.vgcm_alphas = torch.zeros((self.num_envs, 2), dtype=torch.float, device=self.device, requires_grad=False)
        self.vgcm_k_min = torch.zeros((self.num_dof,), dtype=torch.float, device=self.device, requires_grad=False)
        self.vgcm_k_max = torch.zeros_like(self.vgcm_k_min)
        self.vgcm_x_min = torch.zeros_like(self.vgcm_k_min)
        self.vgcm_x_max = torch.zeros_like(self.vgcm_k_min)
        self.vgcm_idxs = [0, 1, 2, 4, 5, 6]

        self.vgcm_k_min[self.vgcm_idxs] = torch.tensor(self.cfg.init_state.k_min, dtype=torch.float, device=self.device, requires_grad=False)
        self.vgcm_k_max[self.vgcm_idxs] = torch.tensor(self.cfg.init_state.k_max, dtype=torch.float, device=self.device, requires_grad=False)
        self.vgcm_x_min[self.vgcm_idxs] = torch.tensor(self.cfg.init_state.x_min, dtype=torch.float, device=self.device, requires_grad=False)
        self.vgcm_x_max[self.vgcm_idxs] = torch.tensor(self.cfg.init_state.x_max, dtype=torch.float, device=self.device, requires_grad=False)
        self.vgcm_k_range = self.vgcm_k_max - self.vgcm_k_min
        self.vgcm_x_range = self.vgcm_x_max - self.vgcm_x_min

        # joint positions offsets and PD gains also VGCM stuff
        self.default_dof_pos = torch.zeros(self.num_dof, dtype=torch.float, device=self.device, requires_grad=False)
        self.default_vgcm_k = torch.zeros(self.num_dof, dtype=torch.float, device=self.device, requires_grad=False)
        self.default_vgcm_x = torch.zeros(self.num_dof, dtype=torch.float, device=self.device, requires_grad=False)
        for i in range(self.num_dofs):
            name = self.dof_names[i]
            angle = self.cfg.init_state.default_joint_angles[name]
            self.default_dof_pos[i] = angle
            self.default_vgcm_x[i] = angle
            found = False
            if name in self.cfg.init_state.default_vgcm_stiffnesses.keys():
                k = self.cfg.init_state.default_vgcm_stiffnesses[name]
                self.default_vgcm_k[i] = k
            else:
                self.default_vgcm_k[i] = 0.
                self.default_vgcm_x[i] = 0.
        ...
        self.default_vgcm_k = self.default_vgcm_k.unsqueeze(0)
        self.default_vgcm_x = self.default_vgcm_x.unsqueeze(0)
    ...
    def _update_vgcm_cfg(self):
        # Convert VGCM scaling factors from numpy to torch
        self.cfg.noise.noise_scales.vgcm_k = torch.tensor(self.cfg.noise.noise_scales.vgcm_k, dtype=torch.float, device=self.device, requires_grad=False)
        self.cfg.noise.noise_scales.vgcm_x = torch.tensor(self.cfg.noise.noise_scales.vgcm_x, dtype=torch.float, device=self.device, requires_grad=False)
        self.obs_scales.vgcm_k = torch.tensor(self.obs_scales.vgcm_k, dtype=torch.float, device=self.device, requires_grad=False)
        self.obs_scales.vgcm_x = torch.tensor(self.obs_scales.vgcm_x, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _reward_torques(self):
        # Penalize *actuator* torques
        return torch.sum(torch.square(self.actuator_torques), dim=1)
    ...
    def _reward_torque_limits(self):
        ...
        return torch.sum(
            (torch.abs(self.actuator_torques) - self.torque_limits * self.cfg.rewards.soft_torque_limit).clip(min=0.), dim=1)
...
#    def _reward_VGCM_stiffness(self):
#        reward = torch.sum()
#        return reward

#    def _reward_VGCM_equilibrium(self):
#        # Penalise 
#        reward = torch.sum((self.gc_tx[:] - self.gc_x[:]) ** 2, dim=1)
#        return reward
